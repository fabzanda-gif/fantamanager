...
export default function Matrix(){
 ...
 {draw?<div className="decisionRow"><button onClick={()=>decide(false)} disabled={busy} className="passButton"><X size={17}/> PASSA</button><button onClick={()=>decide(true)} disabled={busy} className="acceptButton"><Check size={17}/> ACCETTA</button></div>:<button onClick={doDraw} disabled={busy||!role||!!error} className="drawButton live"><CircleDot size={18}/> {role?"PESCA GIOCATORE":"DRAFT COMPLETATO"}</button>}</div>
 <aside className="matrixRules"><span>PROTOCOLLO MATRICE</span><strong>{accepted.length}/16 in rosa</strong><p>Il ranking influenza la rarità, non garantisce il rendimento. Accettare o passare rimuove comunque il giocatore.</p>{(Object.keys(targets) as Role[]).map(r=><div key={r}><b>{counts[r]}/{targets[r]}</b><small>{r==="P"?"PORTIERI":r==="D"?"DIFENSORI":r==="C"?"CENTROCAMPISTI":"ATTACCANTI"}</small></div>)}</aside></section></main>
}