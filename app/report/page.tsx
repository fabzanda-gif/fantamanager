...
type Stat={player_id:string;player_name:string;rating:number;goals:number;chances:number;recoveries:number;duels:number;progressions:number;dangerous_errors:number;saves:number;goals_conceded:number;interventions:number};
...
export default function Report(){
 ...
 return <main className="reportPage">
  ...
  <div className="reportGrid"><section className="ratings"><div className="sectionTitle">PAGELLE <span>{stats.length} titolari</span></div>
   {stats.map(s=><article key={s.player_id}><b className={"rating "+(s.rating>=7?"good":s.rating<6?"bad":"")}>{Number(s.rating).toFixed(1)}</b><div><strong>{s.player_name}</strong><span>{s.saves||s.interventions?s.saves+" parate · "+s.goals_conceded+" subiti · "+s.interventions+" interventi":(s.goals?s.goals+" gol · ":"")+s.recoveries+" rec · "+s.duels+" duelli · "+s.progressions+" prog."}</span></div>{s.dangerous_errors>0&&<em>ERRORE</em>}</article>)}
