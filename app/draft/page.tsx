...
import {ArrowLeft,Check,CircleDot,Loader2,Sparkles,ArrowRight} from "lucide-react";
import {createClient} from "@supabase/supabase-js";import{PLAYER_PHOTOS}from"../player-photos";
...
function drawPhoto(p:Draw){return PLAYER_PHOTOS[p.player_id]||(p.player_name.toUpperCase().includes("GANDELMAN")?"/assets/players/lecce/fm_42094193__GANDELMAN.png":undefined)}

export default function Draft(){
 const[run,setRun]=useState(""),[club,setClub]=useState(""),[round,setRound]=useState(0),[done,setDone]=useState(0),[role,setRole]=useState<Role>("A"),[draw,setDraw]=useState<Draw|null>(null),[justAccepted,setJustAccepted]=useState(false),[busy,setBusy]=useState(true),[error,setError]=useState("");
 ...
 useEffect(()=>{(async()=>{const id=new URLSearchParams(location.search).get("run")||"";setRun(id);if(!id){setError("Run mancante.");setBusy(false);return}
   ...
   setBusy(false)})()},[]);
 async function fish(){if(!run||available<=0)return;setJustAccepted(false);setBusy(true);setError("");const r=await sb.rpc("game_season_draw",{p_run_id:run,p_role:role});if(r.error){setError(r.error.message);setBusy(false);return}const x=r.data?.[0];if(x)setDraw(x as Draw);setBusy(false)}
 async function accept(){if(!draw)return;setBusy(true);const r=await sb.rpc("game_accept_season_draw",{p_run_id:run,p_draw_number:draw.draw_number});if(r.error){setError(r.error.message);setBusy(false);return}setDone(x=>x+1);setDraw(null);setJustAccepted(true);setBusy(false)}
 ...
 {draw?<button className="matrixAcceptButton" disabled={busy} onClick={accept}><Check size={17}/> AGGIUNGI ALLA ROSA</button>:justAccepted?<Link className="matrixContinueButton" href={"/stagione?run="+run}>CONTINUA LA STAGIONE <ArrowRight size={17}/></Link>:<button className="drawButton live" disabled={busy||available<=0} onClick={fish}><CircleDot size={18}/> PESCA {roleName[role]}</button>}</div>
