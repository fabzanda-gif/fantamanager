...
export function buildMatchEvents(run:string,round:number,style:string,opp:string,ps:Starter[],oppPs:Starter[]=[],dryStreak=0){
 ...
 const finish=(us:boolean,minute:number,shotter:Starter|null,creator:Starter|null,contextBoost=0,kind="azione")=>{
   ...
   if(goal){if(us)usGoals++;else opGoals++;add(minute,'goal',us?'user':'opp',shotter,(us?'GOL! ':'GOL '+opp+'! ')+(shotter?.player_name||opp)+' chiude '+kind+' con freddezza: conclusione da '+xg.toFixed(2)+' xG.',{assist:creator&&creator.player_id!==shotter?.player_id?creator:null,xg,shotOnTarget:true});return}
   if(onTarget){add(minute,'shot_saved',us?'user':'opp',shotter,(shotter?.player_name||opp)+' arriva alla conclusione dopo '+kind+', ma il portiere legge il tiro e respinge.',{assist:us&&creator?.player_id!==shotter?.player_id?creator:null,xg,shotOnTarget:true});if(!us){const gk=ps.find(p=>p.role==='P');if(gk)add(minute+1,'keeper_save','user',gk,gk.player_name+' completa l’intervento, blocca la seconda palla e fa ripartire la squadra.')}}
   else add(minute,'shot_missed',us?'user':'opp',shotter,(shotter?.player_name||opp)+' completa '+kind+' con il tiro, ma la conclusione esce dallo specchio.',{assist:us&&creator?.player_id!==shotter?.player_id?creator:null,xg,shotOnTarget:false});
 };
