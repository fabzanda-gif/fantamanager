...
export function buildMatchEvents(run:string,round:number,style:string,opp:string,ps:Starter[],oppPs:Starter[]=[],dryStreak=0){
 const rnd=seeded(`${run}|g${round}|${style}|match5`),out:MatchEvent[]=[];
 ...
 const add=(minute:number,type:string,side:'user'|'opp',player:Starter|null|undefined,desc:string,extra:any={})=>out.push({minute:Math.min(89,minute),type,side,player:player||null,desc,...extra});
 ...
 const finish=(us:boolean,minute:number,shotter:Starter|null,creator:Starter|null,contextBoost=0,kind="azione")=>{
   const trailing=usGoals<opGoals,late=minute>70,base=.045+rnd()*.15,xg=Number(clamp(base+contextBoost+(late&&trailing?.035:0)+streakBoost*.1,.025,.62).toFixed(3)),q=quality(shotter),onTarget=rnd()<clamp(.34+xg*.52+(q-1)*.11,.28,.72),goal=rnd()<clamp(xg*q*(1.02+streakBoost*.6),.015,.74);
   if(goal){if(us)usGoals++;else opGoals++;add(minute,'goal',us?'user':'opp',shotter,(us?'GOL! ':'GOL '+opp+'! ')+(shotter?.player_name||opp)+' trova il varco e manda il pallone in rete! Lo stadio esplode mentre i compagni corrono ad abbracciarlo. Chance da '+xg.toFixed(2)+' xG.',{assist:creator&&creator.player_id!==shotter?.player_id?creator:null,xg,shotOnTarget:true});return}
   if(onTarget){add(minute,'shot_saved',us?'user':'opp',shotter,(shotter?.player_name||opp)+' calcia nello specchio. Il portiere legge la traiettoria e respinge.',{assist:us&&creator?.player_id!==shotter?.player_id?creator:null,xg,shotOnTarget:true});if(!us){const gk=ps.find(p=>p.role==='P');if(gk)add(minute+1,'keeper_save','user',gk,gk.player_name+' completa l’intervento, blocca la seconda palla e fa ripartire la squadra.')}}
   else add(minute,'shot_missed',us?'user':'opp',shotter,(shotter?.player_name||opp)+' prova la conclusione, ma il pallone termina fuori dallo specchio.',{assist:us&&creator?.player_id!==shotter?.player_id?creator:null,xg,shotOnTarget:false});
 };
